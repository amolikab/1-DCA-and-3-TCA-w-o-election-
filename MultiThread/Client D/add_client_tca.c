...
#define OLD_CAFILE "old_cafile.pem"
...
int issue_cert (SSL *ssl,char *port)
{
    ...
    if(z==1)
    {
        ...
        system(test); 
        /*//rename CAFILE to temp name to add the new CA cert to CAFILE
        test[0] = '\0';
        snprintf(test,sizeof(test),"mv %s %s ",CAFILE, OLD_CAFILE);    
        system(test);
        //adding new CA to CAfile
        test[0] = '\0';
        snprintf(test,sizeof(test),"cat %s %s > %s ",CA_CERTIFICATE, OLD_CAFILE, CAFILE);    
        system(test);*/
    }
    else
        printf("Not Verified\n");
