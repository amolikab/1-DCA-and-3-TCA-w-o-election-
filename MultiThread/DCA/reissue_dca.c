...
int reissue_dca(SSL *ssl)
{
    ...
    char test[500]; 
    snprintf(test,sizeof(test),"openssl x509 -req -in %s -passin pass:clientakey -sha1 -extfile myopenssl.cnf -extensions usr_cert_grpB -CA %s -CAkey %s -set_serial 0x%s -out %s",CLIENT_CSR,CERTFILE,KEYFILE,serial,CLIENT_CERT);
    system(test);
