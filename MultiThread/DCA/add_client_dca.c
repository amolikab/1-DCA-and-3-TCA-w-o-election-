...
int issue_cert(SSL *ssl, char *port)
{
    ...
    char test[500]; 
    snprintf(test,sizeof(test),"openssl x509 -req -in %s -passin pass:clientakey -sha1 -extfile myopenssl.cnf -extensions usr_cert_grpB -CA %s -CAkey %s -set_serial 0x%d -out %s",CLIENT_CSR,CERTFILE,KEYFILE,serial_cert,CLIENT_CERT);
    system(test);
